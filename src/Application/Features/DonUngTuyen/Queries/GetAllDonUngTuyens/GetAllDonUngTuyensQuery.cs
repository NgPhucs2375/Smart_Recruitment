...
public class GetAllDonUngTuyensQueryHandler(IApplicationDbContext context, IMapper mapper) 
    : IRequestHandler<GetAllDonUngTuyensQuery, Response<List<GetAllDonUngTuyensViewModel>>>
{
    public async Task<Response<List<GetAllDonUngTuyensViewModel>>> Handle(GetAllDonUngTuyensQuery request, CancellationToken cancellationToken)
    {
        ...
        return new Response<List<GetAllDonUngTuyensViewModel>>(
            mapper.Map<List<GetAllDonUngTuyensViewModel>>(result)
        );
    }
}