...
const INK = "#1f2a37";
const SUBTLE = "#5b6470";
const MESH = "linear-gradient(135deg, #34d399, #60a5fa, #a78bfa)";
const PILL = "linear-gradient(90deg, #34d399, #60a5fa)";
    ...
    l.includes("mail") || l.includes("email")
      ...
      : l.includes("điện") || l.includes("phone") || l.includes("sđt")
        ...
        : l.includes("địa chỉ") || l.includes("address")
          ? MapPin
          : Globe;
  return <Icon className="mt-0.5 size-3.5 shrink-0" style={{ color: "#60a5fa" }} strokeWidth={2} />;
}

function MeshTitle({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="mb-2.5 flex items-center gap-2 text-[12px] font-bold uppercase tracking-[0.14em]" style={{ color: INK }}>
      <span className="inline-block size-2.5 rounded-full" style={{ background: MESH }} />
      {children}
    </h2>
  );
}

export function AuroraMeshTemplate({ data }: ResumeTemplateProps) {
  if (!data.hasContent) {
    return (
  ...
  return (
    <div className="cv-paper cv-paper-a4 bg-white text-[13px] leading-relaxed" style={{ color: INK }}>
      <header className="px-7 pb-6 pt-7" style={{ background: MESH }}>
        ...
        {data.title && <p className="mt-1 text-[13px] font-medium text-white/90">{data.title}</p>}
      ...
      <div className="space-y-5 bg-white px-7 py-6">
        ...
        {data.experience.length > 0 && (
          <SectionShell>
            <MeshTitle>Kinh nghiệm</MeshTitle>
                ...
                <div key={job.id} className="cv-section-item">
                  <div className="flex items-baseline justify-between gap-3">
                    <h3 className="flex items-center gap-1.5 font-bold" style={{ color: INK }}>
                      <Briefcase className="size-3.5 shrink-0" style={{ color: "#60a5fa" }} strokeWidth={2} />
                      {job.role || "Chức danh"}
                    </h3>
        ...
        {data.skills.length > 0 && (
          <SectionShell>
            <MeshTitle>Kỹ năng</MeshTitle>
            <div className="flex flex-wrap gap-1.5">
              {data.skills.map((s) => (
                <span
                  key={s.id}
                  className="cv-section-item rounded-full px-2.5 py-1 text-[12px] font-semibold text-white"
              ...
              {data.certificates.map((c) => (
                <div key={c.id} className="cv-section-item">
                  <p className="flex items-start gap-1.5 text-[12px] font-semibold" style={{ color: INK }}>
                    <Award className="mt-0.5 size-3.5 shrink-0" style={{ color: "#60a5fa" }} strokeWidth={2} />
                    {c.name || "Chứng chỉ"}
                  </p>
