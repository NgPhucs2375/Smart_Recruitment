  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            >
              {resolveBannerTitle(data, "Bản vẽ nhân sự — Rev 01")}
            </p>
            <h1
      ...
      <div className="mt-5 space-y-6">
        {data.summary && (
          <SectionShell>
            <StencilLabel code="SEC.00">Thông tin chuyên môn</StencilLabel>
            <div
              className="cv-section-item rounded-sm border border-dashed bg-white px-4 py-3"
              style={{ borderColor: STEEL }}
            >
              <SplitBlocks text={data.summary} className="whitespace-pre-line break-words [overflow-wrap:anywhere] text-[12.5px]" style={{ color: STEEL }} />
            </div>
          </SectionShell>
        )}
        ...
        {data.experience.length > 0 && (
          <SectionShell>
            <StencilLabel code="SEC.01">Kinh nghiệm làm việc</StencilLabel>
                  ...
                  {data.experience.map((job) => (
                    <tr key={job.id} className="cv-section-item border-t" style={{ borderColor: HAIRLINE }}>
                      <td className="border-r px-3 py-2.5 align-top" style={{ borderColor: HAIRLINE }}>
                        ...
                        {job.description && (
                          <SplitBlocks text={job.description} className="mt-1 whitespace-pre-line break-words [overflow-wrap:anywhere] text-[12px]" style={{ color: MUTED }} />
                        )}
                      </td>
        ...
        {data.projects.length > 0 && (
          <SectionShell>
            <StencilLabel code="SEC.02">Dự án tiêu biểu</StencilLabel>
            <div className="grid grid-cols-2 gap-2.5">
              {data.projects.map((p, idx) => (
                <div
                  key={p.id}
                  className="cv-section-item min-w-0 rounded-sm border bg-white px-3.5 py-3"
                  style={{ borderColor: STEEL }}
                >
                  ...
                  {p.description && (
                    <SplitBlocks text={p.description} className="mt-1 whitespace-pre-line break-words [overflow-wrap:anywhere] text-[12px]" style={{ color: MUTED }} />
                  )}
                  {p.tech.length > 0 && (
              ...
              {data.education.map((edu) => (
                ...
                >
                  <div>
                    ...
                    {edu.description && (
                      <SplitBlocks text={edu.description} className="mt-0.5 whitespace-pre-line break-words [overflow-wrap:anywhere] text-[12px]" style={{ color: MUTED }} />
                    )}
                  </div>
        ...
        {data.skills.length > 0 && (
          <SectionShell>
            <StencilLabel code="SEC.04">Kỹ năng</StencilLabel>
        ...
        {data.certificates.length > 0 && (
          <SectionShell>
            <StencilLabel code="SEC.05">Chứng chỉ</StencilLabel>
            <div className="space-y-2">
      ...
      <footer className="mt-6 flex items-center justify-between border-t-2 pt-3" style={{ borderColor: STEEL_DARK }}>
        <p className="text-[10px] font-bold uppercase min-w-0" style={{ color: STEEL, letterSpacing: "0.24em" }}>
          Khung thông tin — Tỷ lệ 1:1
        </p>
        <p className="text-[10px] font-bold uppercase min-w-0" style={{ color: ACCENT, letterSpacing: "0.24em" }}>
          Duyệt ✓
        </p>
      </footer>
    </div>
