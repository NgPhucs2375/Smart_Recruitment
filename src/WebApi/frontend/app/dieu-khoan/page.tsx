  ...
  return (
    <main className="min-h-dvh bg-background text-foreground">
      ...
      <div className="mx-auto max-w-4xl px-6 py-12 sm:px-10">
        <div className="inline-flex items-center gap-2 rounded-full border border-border/15 bg-white px-3 py-1 text-xs font-medium"><FileText className="size-3.5" /> Điều khoản dịch vụ • Cập nhật 04/09/2026</div>
        <h1 className="mt-6 text-4xl font-medium tracking-[-0.05em]">Điều khoản dịch vụ</h1>
        <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">Áp dụng cho ứng viên và nhà tuyển dụng sử dụng sàn giao dịch việc làm HIREAI. Việc tạo tài khoản đồng nghĩa chấp nhận toàn bộ điều khoản dưới đây.</p>

        <div className="mt-10 space-y-8 rounded-2xl border border-border/10 bg-white p-8 shadow-sm">
          <section>
            <h2 className="text-lg font-semibold">1. Định nghĩa & Phạm vi</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">HIREAI là sàn TMĐT việc làm ngành IT, cung cấp dịch vụ kết nối, AI matching, quản lý hồ sơ và tin tuyển dụng. Người dùng gồm Ứng viên (cá nhân tìm việc) và Nhà tuyển dụng (doanh nghiệp, tổ chức).</p>
          </section>
          <section>
          ...
          <section>
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Ứng viên miễn phí 100%. Nhà tuyển dụng trả phí theo gói hiển thị công khai tại /bang-gia. HIREAI không đảm bảo số lượng ứng viên cố định.</p>
          </section>
          <section>
            <h2 className="text-lg font-semibold">4. Quyền sở hữu & AI</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Thuật toán AI chỉ hỗ trợ đề xuất; quyết định tuyển dụng thuộc về người dùng. Dữ liệu huấn luyện được ẩn danh và tuân thủ <Link href="/bao-mat#du-lieu-ca-nhan" className="underline">Chính sách bảo vệ dữ liệu cá nhân</Link>.</p>
          </section>
