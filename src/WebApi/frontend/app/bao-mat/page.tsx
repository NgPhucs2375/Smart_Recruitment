  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-4xl px-6 py-12 sm:px-10">
        <div className="inline-flex items-center gap-2 rounded-full border border-border/15 bg-white px-3 py-1 text-xs font-medium"><Lock className="size-3.5" /> Chính sách bảo mật • Nghị định 13/2023/NĐ-CP</div>
        <h1 className="mt-6 text-4xl font-medium tracking-[-0.05em]">Chính sách bảo mật &amp; Dữ liệu cá nhân</h1>
        <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">HIREAI tuân thủ Luật Bảo vệ Dữ liệu cá nhân, lưu trữ tại Việt Nam, mã hóa TLS 1.3, và cho phép người dùng kiểm soát toàn bộ dữ liệu.</p>
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs">
          ...
          <a href="#cookies" className="rounded-full border border-border/15 bg-white px-3 py-1.5">Cookie</a>
          <a href="#quyen" className="rounded-full border border-border/15 bg-white px-3 py-1.5">Quyền của bạn</a>
        </div>

        <div className="mt-8 space-y-8 rounded-2xl border border-border/10 bg-white p-8 shadow-sm">
          ...
          <section>
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">TLS 1.3, HSTS, mã hóa mật khẩu Argon2, token JWT ngắn hạn + refresh rotation, audit log, SOC 2 Type II. Báo lỗ hổng: security@hireai.vn.</p>
          </section>
          ...
          <section id="cookies" className="scroll-mt-8">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Chỉ dùng cookie cần thiết (đăng nhập, bảo mật) và analytics ẩn danh. Không cookie quảng cáo bên thứ ba. Bạn có thể tắt tại trình duyệt; một số tính năng (giữ đăng nhập) sẽ hạn chế.</p>
          </section>

          <section id="quyen" className="scroll-mt-8">
            <h2 className="text-lg font-semibold">Quyền & Khiếu nại</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Gửi yêu cầu tới <b>dpo@hireai.vn</b> hoặc <Link href="/lien-he" className="underline">Liên hệ</Link>. Nếu không hài lòng, bạn có quyền khiếu nại tới Cục An toàn thông tin hoặc khởi kiện theo pháp luật VN.</p>
          </section>

          <section className="rounded-xl border border-border/10 bg-background p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">Mọi thay đổi sẽ thông báo qua email và banner trong 14 ngày trước hiệu lực.</p>
          </section>
        </div>
