  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-5 px-4 py-6 sm:px-6 sm:py-8">
      ...
      {loading ? (
        ...
        <article className="overflow-hidden rounded-3xl border border-linen bg-card shadow-sm">
          <div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-start">
            <span
              aria-hidden="true"
              className="flex size-16 shrink-0 items-center justify-center rounded-2xl bg-frost font-mono text-lg font-bold text-navy"
            >
            ...
            <div className="min-w-0 flex-1">
              ...
              {company.linhVucHoatDong && (
                <p className="mt-1 text-sm text-charcoal/60">{company.linhVucHoatDong}</p>
              )}
          ...
          {company.moTa && (
            <div className="border-t border-border/60 p-6">
              <h2 className="text-sm font-semibold text-charcoal">Giới thiệu</h2>
              <p className="mt-2 whitespace-pre-line text-sm leading-6 text-charcoal/70">{company.moTa}</p>
            </div>
          )}
        </article>
      )}
