...
import { CvPreview } from "@/features/tao-cv/components/cv-preview";
import { TEMPLATE_REGISTRY, DEFAULT_TEMPLATE_ID } from "@/features/tao-cv/templates/registry";
import { defaultCvData } from "@/features/tao-cv/constants";
import type { CvFormData } from "@/features/tao-cv/types";
import { toCvPayload } from "@/features/tao-cv/types";
  ...
  async function persistAndGo(edit: boolean) {
    ...
    try {
      ...
      const id = await cvApi.createCv(
        toCvPayload(hs.id, liveCv, true) as unknown as Record<string, unknown>
      );
