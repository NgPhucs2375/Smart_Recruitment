...
_services.Configure<ForwardedHeadersOptions>(options =>
{
    ...
    // KHÔNG Clear KnownNetworks/KnownProxies: chỉ áp dụng forwarded headers từ proxy
    // đáng tin (mặc định loopback). TODO pre-deploy: bổ sung IP egress của Render vào
    // KnownProxies/KnownNetworks khi có thông tin xác thực từ nhà cung cấp.
});
...
_services.Configure<SeedSettings>(_config.GetSection("Seed"));
...
_services.AddHealthChecks().AddCheck<DatabaseReadinessHealthCheck>("database");
...
// Liveness: process sống là 200. Readiness database riêng ở /health/ready
// (trả 503 khi DB unreachable, không lộ connection string/exception).
app.MapHealthChecks("/health", new HealthCheckOptions { Predicate = _ => false });
app.MapHealthChecks("/health/ready", new HealthCheckOptions { Predicate = check => check.Name == "database" });
