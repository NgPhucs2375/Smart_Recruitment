...
/// <summary>
/// Mirrors form-filling <c>agent/FileAttachmentMiddleware.cs</c> byte-for-byte
/// in behaviour: extracts AG-UI / OpenAI style file parts from the JSON body
/// into <c>HttpContext.Items["__attachments__"]</c> so agent tools can read them.
/// Register with <c>app.UseMiddleware&lt;FileAttachmentMiddleware&gt;()</c> BEFORE
/// <c>MapAGUIServer</c> / <c>MapControllers</c>.
/// </summary>
public sealed class FileAttachmentMiddleware(RequestDelegate next)
{
    public async Task InvokeAsync(HttpContext context)
    {
            ...
            context.Request.ContentType?.Contains("application/json") == true)
        {
            ...
            if (TryTransform(body, out var transformed, out var attachments))
            {
                ...
                var bytes = System.Text.Encoding.UTF8.GetBytes(transformed);
